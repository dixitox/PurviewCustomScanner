...
> 📌 <b>Note:</b> Although this solution relies on the use of Azure Open AI LLM for the classification activities, you can replace it with your own custom classifier engine (e.g. program using regular expressions, word matching, etc.)
...
## 📊 Understanding Asset Hierarchies in Purview
...
- **SharePoint**: Account → RootFolder → Folder → SubFolder → File
- **FileSystem**: Root → Folder → SubFolder → File
...
SharepointAccount (mngenvmcap802465.sharepoint.com)
  └── SharepointRootFolder (Purview-Sharepoint-Demo)
      └── SharepointFolder (Insurance)
          └── SharepointFolder (Claims)
              ├── SharepointFile (Claim.pdf)
              └── SharepointFile (Policy.docx)
```
...
2. Go to **Data Catalog** → **Browse** → Select your collection (SharePoint or FileSystem)
...
> ⚠️ **Warning**: Cleanup is irreversible. Make sure you've reviewed all assets in Purview before running cleanup.
